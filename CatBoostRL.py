"""
REINFORCEMENT LEARNING: RNN Policy Network → CatBoost Judge

Architecture:
1. RNN processes temporal data → stochastic latent Z (policy, REINFORCE)
2. Concatenate [Static + Last_Values + Z] → CatBoost (reward judge)
3. CatBoost provides per-sample reward; policy is updated via policy gradient
...
def train_policy_with_catboost_reward(
    policy_net,
    train_loader,
    val_loader,
    catboost_params,
    ...
    """Train RNN policy using CatBoost as non-differentiable reward judge.

    Algorithm (REINFORCE):
    1. Sample Z from policy → features = [Static + Last + Z]
    2. Train/update CatBoost on those features
    3. Compute per-sample rewards from CatBoost predictions
    ...
    print("  [RL Training] CatBoost reward judge...")
    ...
    for epoch in range(epochs):
        ...
        if epoch % update_catboost_every == 0 or catboost_model is None:
            catboost_model = CatBoostClassifier(**catboost_params)
            catboost_model.fit(X_train, y_train, verbose=False)

        y_pred_proba = catboost_model.predict_proba(X_train)[:, 1]
        ...
        if (epoch + 1) % 5 == 0:
            ...
            cb_val = CatBoostClassifier(**catboost_params)
            cb_val.fit(X_train, y_train, verbose=False)
            y_val_proba = cb_val.predict_proba(X_val)[:, 1]
...
def main():
    ...
    ax.set_title("RL Policy (RNN + CatBoost)", fontsize=14, fontweight="bold")
    ax.legend(loc="lower right"); ax.grid(alpha=0.3)
    plt.tight_layout()
    os.makedirs("result", exist_ok=True)
    plt.savefig("result/catboost_rl.png", dpi=300)
    print("\nPlot saved to result/catboost_rl.png")
